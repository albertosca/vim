...
| `,at` | Liga/desliga o auto-trigger (fica manual — sem custo de API não-intencional) |
| `,ap` | Alterna o provider ativo entre Gemini e Claude — **só existe se as duas API keys estiverem em `~/.zsh_secrets`** |

> Provider padrão é Gemini (mais barato). Claude via `,ap` é pro quando quiser uma sugestão mais "esperta" numa parte complexa do código, aceitando o custo maior.
...
| `Esc` | Sai do insert mode — a sugestão some sozinha, sem deixar nada no buffer |
| `,ap` | Alterna o provider ativo entre Gemini e Claude — **disponível se `ant` estiver autenticado (assinatura Claude) OU `ANTHROPIC_API_KEY` estiver em `~/.zsh_secrets`** |
