...
## 23. minuet-ai.nvim — autocomplete inline (Gemini/Claude)

| Atalho | Ação |
| :--- | :--- |
| (automático, ghost text) | Sugestão aparece sozinha ~600ms depois de parar de digitar (provider padrão: Gemini) |
| `Ctrl+y` | Aceita a sugestão inteira |
| `Ctrl+e` | Dispensa a sugestão visível |
| `,pt` | Liga/desliga o auto-trigger (fica manual — sem custo de API não-intencional) |
| `,pr` | Alterna o provider ativo entre Gemini e Claude — **só existe se as duas API keys estiverem configuradas** (`~/.config/gemini/vim-ai-autocomplete.env` e `~/.config/anthropic/vim-ai-autocomplete.env`) |

> Provider padrão é Gemini (mais barato). Claude via `,pr` é pro quando quiser uma sugestão mais "esperta" numa parte complexa do código, aceitando o custo maior. Não usa `,a*` de propósito — `,a` já é code actions do CoC, e compartilhar prefixo com um mapeamento completo existente causa disparo do comando errado se você não digitar rápido o suficiente (achado real, reportado em teste manual). Também não usa `,pv` — colide de verdade com o seletor de venv Python (seção 24), que registra `,pv` antes na ordem de carregamento do `init.vim` e seria sobrescrito silenciosamente (achado real, reportado em teste manual: "PV e PT deu pau").
...
> Plugin próprio (`plugins/vim-ai-autocomplete/`), arquitetura inspirada na técnica pública do `github/copilot.vim` (prop_add/Vim 9 textprop) — nenhum código copiado (copilot.vim é "All Rights Reserved"). Mesma UX do `minuet-ai.nvim` (seção 23, Neovim): Gemini padrão, Claude via toggle, mesmas API keys (`~/.config/<provedor>/vim-ai-autocomplete.env`, lidas por `configs.vim` e setadas so no processo do Vim/Neovim — nao mais export global em `~/.zsh_secrets`). Claude sempre via `ANTHROPIC_API_KEY` estática — o CLI `ant` (OAuth) foi cogitado e removido: ele cobra do mesmo credito de API pago por token, sem dar acesso ao uso incluso da assinatura Claude Pro/Max (achado real, 2026-07-20).
>
