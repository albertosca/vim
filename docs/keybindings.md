...
> Plugin próprio (`plugins/vim-ai-autocomplete/`), arquitetura inspirada na técnica pública do `github/copilot.vim` (prop_add/Vim 9 textprop) — nenhum código copiado (copilot.vim é "All Rights Reserved"). Mesma UX do `minuet-ai.nvim` (seção 23, Neovim): Gemini padrão, Claude via toggle, mesmas API keys em `~/.zsh_secrets`.
