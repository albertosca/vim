...
| `,pv` | Alterna o provider ativo entre Gemini e Claude — **só existe se as duas API keys estiverem em `~/.zsh_secrets`** |

> Provider padrão é Gemini (mais barato). Claude via `,pv` é pro quando quiser uma sugestão mais "esperta" numa parte complexa do código, aceitando o custo maior. Não usa `,a*` de propósito — `,a` já é code actions do CoC, e compartilhar prefixo com um mapeamento completo existente causa disparo do comando errado se você não digitar rápido o suficiente (achado real, reportado em teste manual).
...
| `,pv` | Alterna o provider ativo entre Gemini e Claude — **disponível se `ant` estiver autenticado (assinatura Claude) OU `ANTHROPIC_API_KEY` estiver em `~/.zsh_secrets`** |
