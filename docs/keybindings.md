...
| `,pr` | Alterna o provider ativo entre Gemini e Claude — **só existe se as duas API keys estiverem em `~/.zsh_secrets`** |
...
| `,pt` | Liga/desliga o auto-trigger (fica manual — sem custo de API não-intencional) |
| `,pr` | Alterna o provider ativo entre Gemini e Claude — **disponível se `ant` estiver autenticado (assinatura Claude) OU `ANTHROPIC_API_KEY` estiver em `~/.zsh_secrets`** |

> Plugin próprio (`plugins/vim-ai-autocomplete/`), arquitetura inspirada na técnica pública do `github/copilot.vim` (prop_add/Vim 9 textprop) — nenhum código copiado (copilot.vim é "All Rights Reserved"). Mesma UX do `minuet-ai.nvim` (seção 23, Neovim): Gemini padrão, Claude via toggle, mesmas API keys em `~/.zsh_secrets`. **Claude prefere a assinatura** (via `ant auth login`, se o `ant` CLI estiver instalado e autenticado) **antes de cair pra `ANTHROPIC_API_KEY` estática** — ver `docs/setup.md` pra instalar o `ant`.
